#include "../src_new/MurmurHash.h"
#include "../src_new/kos.h"
#include "../src_new/Party.h"
#include "../src_new/Functions.h"
#include "../src_new/P0.h"
#include "../src_new/Pi.h"
#include "../src_new/Bitstring.h"
...
int main(int argc, char** argv)
{
	...
	//if p0
	    ...
	    unsigned int* hash_seeds=functions::get_seeds(seeds_num,seed);
	     ...
	     for (int i=0;i<(parties-1);i++){
	        ...
	        threads.push_back(new thread(functions::offline_apport_receiver,P0_s[i],&ips,ports,player,other_player));
	      ...
	      //--steps 7+10-- online apport 
              ...
              //online apport as a receiver-creating function according to the BF bits and sending it to the other party

              cout<<"online apport"<<endl;

	      for (int i=0;i<(parties-1);i++){
                threads.push_back(new thread(functions::online_apport_receiver,P0_s[i],test));
	      }
	      ...
	      threads.clear();
              ...
              block** keys_0=new block* [parties-1];
              for (int i=0;i<(parties-1);i++)
                 keys_0[i]=new block[items_size];

	      //--step-- computing the keys
              //going over h* of each party and xoring the strings of each item
	      //for every instace of p0 we will get array of keys in the size of the DB
              cout<<"compute keys"<<endl;
	      for (int i=0;i<(parties-1);i++){
                  threads.push_back(new thread(functions::compute_keys_p0,keys_0[i],h_kokhav,items_size,P0_s[i]));
	       }
	       ...
	       block* keyss=new block[items_size];
		
	       for (int i=0;i<items_size;i++)
			keyss[i]=toBlock(0,0);


	       //--step -- xoring the arrays of keys
               //go over the arrays of keys from earlier and xoring every key in place i with the other keys in place i
		
               cout<<"getting keys p0"<<endl;		
	       for (int i=0;i<(parties-1);i++){
                   cout<<"party "<<(i+1)<<endl;
		   for (int j=0;j<items_size;j++){
                        keyss[j]=keyss[j]^keys_0[i][j];
		   }
	           delete [] keys_0[i];
	        }
	        ...
	        block** keyss_recv=new block*[(parties)];
	        keyss_recv[0]=keyss;

	        //--step -- getting keys from the other pi's
                cout<<"getting keys pi"<<endl;    
	        for (int i=1;i<parties;i++){
                     keyss_recv[i]=new block[items_size];
                     threads.push_back(new thread(functions::recv_keys,P0_s[i-1],keyss_recv[i]));
	        }
		...
		int arr1_size=int(pow(items_size,mid));
		block* arr1=new block[arr1_size];
		int* items_ind=new int[arr1_size];
		int arr2_size=int(pow(items_size,mid));
                block* arr2=new block[arr2_size];

	        //--step -- xoring all the keys	into 2 groups	
                threads.push_back(new thread( functions::xor_2,mid,keyss_recv, arr1, items_ind,items_size, 0, 0, 0,toBlock(0,0)));
                threads.push_back(new thread(functions::xor_3,parties, keyss_recv, arr2,items_size, 0, mid,toBlock(0,0)));
 	        ...
 	        //--step -- sorting the two arrays of the xored keys           
                cout<<"merge sort"<<endl;
		thread* t1=new thread( functions::merge_sort1,arr2,0,arr2_size);
                thread* t2=new thread( functions::merge_sort2,arr1,0,arr1_size,items_ind);

                t1->join();t2->join();
                delete t1; delete t2;

		//--step-- going over the 2 arrays and finding the xored keys which are similar
	        cout<<"getting intersection"<<endl;
                int i=0,j=0;
                while ((i<arr1_size) && (j<arr2_size)){
                      if (arr1[i]==arr2[j]){
                         cout<<items[items_ind[i]]<<" ";
                         i++;j++;
                      }
	...
	// if pi
	    ...
	    //--steps 3+4-- offline_apport
	       ...
	       //running offline apport as a sender
	       ...
	       thread* t1=new thread(functions::offline_apport_sender,pi,&ips,ports,player,0);
	       ...
	       thread* t2=new thread(functions::secret_sharing_aes,parties,Nbf,test,keys,keys_recv);
            ...
            {
               cout<<"Offline_app_ROT executed"<<endl;
               functions::offline_apport_sender(pi,&ips,ports,player,0);
            }
	    ...
	    pi->create_BF(h_kokhav,hash_seeds, seed);
            ...
            //run online apport as a sender-receiving the function
            functions::online_apport_sender(pi,test);
	    ...
	    //--step -- computing the keys and sending them to P0
            //go over each h* of each item and get its key by xoring the strings
	    //finally, sends the keys to P0
            cout<<"computing keys"<<endl;

            block* keys_i=new block[items_size];
            functions::compute_keys_pi(keys_i,h_kokhav,items_size,pi);
